...
Kai.createDatePicker = function(year, month, day = 1, selectCb, $router) {
  ...
  return new Kai({
    ...
    <div class="kui-option-menu">\
      <div class="kui-option-title">{{ title }}</div>\
      <div class="kui-option-body" style="margin:0;padding:0;">\
        <div style="display:flex;flex-direction:row;justify-content:space-between;align-items:center;padding:10px 0px;">\
          <div id="day" style="width:32%;text-align:center;">\
            <div style="color:#323232;padding:5px 0;">{{ dayT }}</div>\
            <div id="__kai_dp_day__" style="color:#873eff;font-size:18px;padding:5px 0;">{{ dayM }}</div>\
            <div style="color:#323232;padding:5px 0;">{{ dayB }}</div>\
          </div>\
          <div id="month" style="width:36%;text-align:center;border-style:solid;border-width:1px;border-color:#fff #e6e6e6 #fff #e6e6e6;">\
            <div style="color:#323232;padding:5px 0;">{{ monthT }}</div>\
            <div id="__kai_dp_month__" style="color:#873eff;font-size:18px;padding:5px 0;">{{ monthM }}</div>\
            <div style="color:#323232;padding:5px 0;">{{ monthB }}</div>\
          </div>\
          <div id="year" style="width:32%;text-align:center;">\
            <div style="color:#323232;padding:5px 0;">{{ yearT }}</div>\
            <div id="__kai_dp_year__" style="color:#873eff;font-size:18px;padding:5px 0;">{{ yearM }}</div>\
            <div style="color:#323232;padding:5px 0;">{{ yearB }}</div>\
          </div>\
        </div>\
      </div>\
    </div>',
